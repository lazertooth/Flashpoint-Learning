...
export const validateQuestionName = (title, question, quizzes) => {
	...
	return challenges.reduce((answer, challenge) => {
		...
		const challengeTitle = challenge.get('id');
 ...
 * the title property content to be the challenge id. */
export const setHtmlMetadata = (id) => {
	var elements = document.getElementsByTagName('meta');
	for (var i = 0; i < elements.length; i++) {
	  if (elements[i].getAttribute("property") === "og:title") {
	     elements[i].setAttribute('content', id);
	  }
	}
};
